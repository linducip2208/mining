...
Route::middleware(['auth'])->group(function () {
    ...
    Route::post('/approvals/{action}/act', [ApprovalCenterController::class, 'act'])->name('approval.act')->middleware('can:approval.approve');
    ...
    Route::put('roles/{role}/permissions', [RoleController::class, 'updatePermissions'])->name('role.update-permissions')->middleware('can:role.update');
    Route::resource('roles', RoleController::class)->except(['index', 'show'])->names(['create' => 'role.create', 'store' => 'role.store', 'edit' => 'role.edit', 'update' => 'role.update', 'destroy' => 'role.destroy'])->middleware('can:role.create');
    ...
    Route::put('settings', [SettingController::class, 'update'])->name('setting.update')->middleware('can:setting.update');
    ...
    Route::post('attendances/import', [AttendanceController::class, 'import'])->name('attendances.import')->middleware('can:attendance.create');
    ...
    Route::post('leaves/{leave}/approve', [LeaveController::class, 'approve'])->name('leaves.approve')->middleware('can:leave.approve');
    Route::post('overtimes/{overtime}/approve', [OvertimeController::class, 'approve'])->name('overtimes.approve')->middleware('can:overtime.approve');
    Route::resource('payroll-runs', PayrollViewController::class)->middleware('permission:payroll.view');
    Route::post('payroll-runs/{payroll_run}/calculate', [PayrollViewController::class, 'calculate'])->name('payroll.calculate')->middleware('can:payroll.update');
    Route::post('payroll-runs/{payroll_run}/approve', [PayrollViewController::class, 'approve'])->name('payroll.approve')->middleware('can:payroll.approve');
    Route::post('payroll-runs/{payroll_run}/post', [PayrollViewController::class, 'post'])->name('payroll.post')->middleware('can:payroll.post');
    Route::post('payroll-runs/{payroll_run}/pay', [PayrollViewController::class, 'pay'])->name('payroll.pay')->middleware('can:payroll.post');
    ...
    Route::post('operator-incentives/{operator_incentive}/approve', [OperatorIncentiveController::class, 'approve'])->name('operator-incentives.approve')->middleware('can:incentive.approve');

    // ===== MINING =====
    Route::resource('mining-activities', MiningActivityController::class)->middleware('permission:mining.view');
    Route::post('mining-activities/{mining_activity}/submit', [MiningActivityController::class, 'submit'])->name('mining.submit')->middleware('can:mining.update');
    Route::post('mining-activities/{mining_activity}/approve', [MiningActivityController::class, 'approve'])->name('mining.approve')->middleware('can:mining.approve');
    Route::post('mining-activities/{mining_activity}/post', [MiningActivityController::class, 'post'])->name('mining.post')->middleware('can:mining.post');

    // ===== WEIGHBRIDGE =====
    Route::resource('weighbridge-tickets', WeighbridgeTicketController::class)->middleware('permission:weighbridge.view');
    Route::post('weighbridge-tickets/first-weigh', [WeighbridgeTicketController::class, 'firstWeigh'])->name('weighbridge.first')->middleware('can:weighbridge.create');
    Route::post('weighbridge-tickets/{weighbridge_ticket}/second-weigh', [WeighbridgeTicketController::class, 'secondWeigh'])->name('weighbridge.second')->middleware('can:weighbridge.create');
    Route::post('weighbridge-tickets/{weighbridge_ticket}/override', [WeighbridgeTicketController::class, 'overrideWeight'])->name('weighbridge.override')->middleware('can:weighbridge.update');
    Route::post('weighbridge-tickets/{weighbridge_ticket}/post', [WeighbridgeTicketController::class, 'postTicket'])->name('weighbridge.post')->middleware('can:weighbridge.post');
    Route::post('weighbridge-tickets/{weighbridge_ticket}/void', [WeighbridgeTicketController::class, 'void'])->name('weighbridge.void')->middleware('can:weighbridge.void');
    Route::post('weighbridge-tickets/{weighbridge_ticket}/print', [WeighbridgeTicketController::class, 'printTicket'])->name('weighbridge.print')->middleware('can:weighbridge.print');

    // ===== PRODUCTION =====
    Route::resource('production-batches', ProductionBatchController::class)->middleware('permission:production.view');
    Route::post('production-batches/{production_batch}/submit', [ProductionBatchController::class, 'submit'])->name('production.submit')->middleware('can:production.update');
    Route::post('production-batches/{production_batch}/approve', [ProductionBatchController::class, 'approve'])->name('production.approve')->middleware('can:production.approve');
    Route::post('production-batches/{production_batch}/post', [ProductionBatchController::class, 'post'])->name('production.post')->middleware('can:production.post');
    ...
    Route::post('stock-transfers/{stock_transfer}/post', [StockTransferController::class, 'post'])->name('stock-transfers.post')->middleware('can:stock.post');
    Route::resource('stock-adjustments', StockAdjustmentController::class)->middleware('permission:stock.view');
    Route::post('stock-adjustments/{stock_adjustment}/post', [StockAdjustmentController::class, 'post'])->name('stock-adjustments.post')->middleware('can:stock.post');

    // ===== PROCUREMENT =====
    Route::resource('purchase-requests', PurchaseRequestController::class)->middleware('permission:purchase_request.view');
    Route::post('purchase-requests/{purchase_request}/submit', [PurchaseRequestController::class, 'submit'])->name('purchase-requests.submit')->middleware('can:purchase_request.update');
    Route::post('purchase-requests/{purchase_request}/approve', [PurchaseRequestController::class, 'approve'])->name('purchase-requests.approve')->middleware('can:purchase_request.approve');
    Route::resource('purchase-orders', PurchaseOrderController::class)->middleware('permission:purchase_order.view');
    Route::post('purchase-orders/{purchase_order}/approve', [PurchaseOrderController::class, 'approve'])->name('purchase-orders.approve')->middleware('can:purchase_order.approve');
    Route::resource('goods-receipts', GoodsReceiptController::class)->middleware('permission:goods_receipt.view');
    Route::post('goods-receipts/{goods_receipt}/post', [GoodsReceiptController::class, 'post'])->name('goods-receipts.post')->middleware('can:goods_receipt.post');
    Route::resource('vendor-bills', VendorBillController::class)->middleware('permission:vendor_bill.view');
    Route::post('vendor-bills/{vendor_bill}/post', [VendorBillController::class, 'post'])->name('vendor-bills.post')->middleware('can:vendor_bill.post');
    Route::post('vendor-bills/{vendor_bill}/pay', [VendorBillController::class, 'pay'])->name('vendor-bills.pay')->middleware('can:vendor_bill.post');
    ...
    Route::post('sales-orders/{sales_order}/approve', [SalesOrderController::class, 'approve'])->name('sales-orders.approve')->middleware('can:sales_order.approve');
    Route::resource('delivery-orders', DeliveryOrderController::class)->middleware('permission:delivery_order.view');
    Route::post('delivery-orders/{delivery_order}/complete', [DeliveryOrderController::class, 'complete'])->name('delivery-orders.complete')->middleware('can:delivery_order.update');
    Route::resource('invoices', InvoiceController::class)->middleware('permission:invoice.view');
    Route::post('invoices/{invoice}/post', [InvoiceController::class, 'post'])->name('invoices.post')->middleware('can:invoice.post');
    ...
    Route::post('deposits/in', [CustomerDepositController::class, 'depositIn'])->name('deposit.in')->middleware('can:deposit.create');
    Route::post('deposits/refund', [CustomerDepositController::class, 'refund'])->name('deposit.refund')->middleware('can:deposit.create');
    ...
    Route::post('price-lists/{price_list}/approve', [PriceListController::class, 'approve'])->name('price-lists.approve')->middleware('can:price.approve');
    ...
    Route::post('price-variances/{price_variance}/approve', [PriceVarianceController::class, 'approve'])->name('price-variances.approve')->middleware('can:price_variance.approve');
    ...
    Route::post('work-orders/{work_order}/approve', [WorkOrderController::class, 'approve'])->name('work-orders.approve')->middleware('can:work_order.approve');
    Route::post('work-orders/{work_order}/start', [WorkOrderController::class, 'start'])->name('work-orders.start')->middleware('can:work_order.update');
    Route::post('work-orders/{work_order}/complete', [WorkOrderController::class, 'complete'])->name('work-orders.complete')->middleware('can:work_order.update');
    Route::post('work-orders/{work_order}/parts/{part}/issue', [WorkOrderController::class, 'issuePart'])->name('work-orders.issue-part')->middleware('can:work_order.update');
    ...
    Route::post('journals/{journal}/post', [JournalController::class, 'post'])->name('journals.post')->middleware('can:journal.post');
    Route::post('journals/{journal}/reverse', [JournalController::class, 'reverse'])->name('journals.reverse')->middleware('can:journal.unpost');
    ...
    Route::post('documents/{document}/approve', [DocumentController::class, 'approve'])->name('documents.approve')->middleware('can:document.approve');
    Route::get('documents/{document}/download', [DocumentController::class, 'download'])->name('documents.download');
    Route::resource('csr', CsrController::class)->middleware('permission:csr.view');
    Route::post('csr/{csr}/approve', [CsrController::class, 'approve'])->name('csr.approve')->middleware('can:csr.approve');
    Route::post('csr/{csr}/activities', [CsrController::class, 'addActivity'])->name('csr.activities.add')->middleware('can:csr.update');
    Route::post('csr/{csr}/expenses', [CsrController::class, 'addExpense'])->name('csr.expenses.add')->middleware('can:csr.update');
