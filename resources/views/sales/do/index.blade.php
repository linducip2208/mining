...
<th class="px-4 py-2.5">Customer</th><th class="px-4 py-2.5">Nopol</th><th class="px-4 py-2.5 text-right">Qty</th><th class="px-4 py-2.5">Status</th><th class="px-4 py-2.5">Tiket</th><th></th>
    </x-slot:head>
    <tbody>
        ...
        <tr class="hover:bg-slate-50">
            ...
            <td class="px-4 py-2.5 text-xs">
                @if ($item->ticket)
                    <a href="{{ route('weighbridge-tickets.show', $item->ticket) }}" class="text-amber-600 hover:underline">{{ $item->ticket->ticket_no }}</a>
                @else
        ...
        @empty
        <tr><td colspan="9" class="px-4 py-10 text-center text-slate-400">Belum ada DO</td></tr>
        @endforelse
    </tbody>
