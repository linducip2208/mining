...
<form method="POST" action="{{ route('weighbridge.first') }}" class="bg-white rounded-xl border border-slate-200 p-6 max-w-3xl">
    ...
    <div class="form-actions-sticky flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-white">
        <button class="px-5 py-2.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold min-h-[44px]">Simpan Timbang Pertama</button>
        <a href="{{ route('weighbridge-tickets.index') }}" class="px-5 py-2.5 rounded-lg bg-slate-100 text-sm min-h-[44px] inline-flex items-center">Batal</a>
    </div>
</form>
@endsection