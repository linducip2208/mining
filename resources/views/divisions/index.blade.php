...
<div class="flex items-center justify-between mb-4">
    <div>
        <h1 class="text-xl font-bold text-slate-800">Divisi</h1>
        <p class="text-sm text-slate-500">Kelola data Divisi</p>
    </div>
    @can('divisions.create')
    <x-btn-create label="Tambah Divisi" :href="route('divisions.create')" />
    @endcan
</div>

<x-filter-bar :route="route('divisions.index')">
    ...
    <tbody>
        ...
        <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-4 py-2.5 text-right whitespace-nowrap">
                            @can('divisions.update')<a href="{{ route('divisions.edit', $item) }}" class="text-indigo-600 hover:underline text-xs">Edit</a>@endcan
                            @can('divisions.delete')
                            <form action="{{ route('divisions.destroy', $item) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data ini?')">
                                @csrf @method('DELETE')
