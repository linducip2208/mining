...
@section('content')
<div class="mb-4">
    <h1 class="text-xl font-bold text-slate-800">{{ $item ? 'Edit' : 'Tambah' }} Cabang</h1>
</div>

<form method="POST" action="{{ $item ? route('branches.update', $item) : route('branches.store') }}" class="bg-white rounded-xl border border-slate-200 p-6 max-w-4xl">
    @csrf
    ...
    <div class="flex gap-2 mt-6">
        ...
        <a href="{{ route('branches.index') }}" class="px-5 py-2 rounded-lg bg-slate-100 text-sm">Batal</a>
    </div>
</form>
@endsection