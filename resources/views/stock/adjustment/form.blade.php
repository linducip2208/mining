...
@section('content')
<h1 class="text-xl font-bold text-slate-800 mb-4">Stock Opname / Penyesuaian</h1>
<form method="POST" action="{{ route('stock-adjustments.store') }}" class="bg-white rounded-xl border border-slate-200 p-6 max-w-3xl">
    @csrf
    <div class="grid md:grid-cols-3 gap-4">
        ...
        <div><label class="text-xs font-semibold text-slate-600 uppercase">Tanggal</label>
            <input type="date" name="adjustment_date" value="{{ today()->toDateString() }}" required class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-200 text-sm"></div>
        <div><label class="text-xs font-semibold text-slate-600 uppercase">Tipe</label>
            <select name="type" class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm"><option value="OPNAME">Stock Opname</option><option value="ADJUSTMENT">Penyesuaian</option></select></div>
    </div>
    <div id="lines" class="mt-4 space-y-2">
        <div class="flex gap-2 line">
            ...
            <input name="lines[0][counted_qty]" type="number" step="0.0001" placeholder="Qty hasil hitung" class="w-40 px-3 py-2 rounded-lg border border-slate-200 text-sm">
        </div>
    </div>
    <button type="button" onclick="addLine()" class="mt-2 text-xs text-amber-600">+ Tambah baris</button>
    <div class="mt-4"><label class="text-xs font-semibold text-slate-600 uppercase">Alasan (wajib)</label>
    ...
    <div class="flex gap-2 mt-6">
        ...
        <a href="{{ route('stock-adjustments.index') }}" class="px-5 py-2 rounded-lg bg-slate-100 text-sm">Batal</a>
    </div>
</form>
@push('scripts')
<script>
