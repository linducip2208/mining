        ...
        <details class="group px-5 py-4 hover:bg-amber-50/30 border-l-2 border-transparent open:border-amber-400">
            <summary class="flex flex-wrap items-center gap-3 cursor-pointer list-none">
                ...
                <span class="min-w-0 flex-1">
                    <span class="block text-sm font-semibold truncate">{{ HumanLabel::label($req->module) }} · {{ $req->transaction_number }}</span>
            ...
            <div class="mt-3 ml-1">
                <x-ui.timeline :items="$steps->map(fn ($s) => ['title' => 'Tahap ' . $s->sequence . ' — ' . ($s->approver?->name ?? '—'), 'body' => StatusLabel::label($s->action) . ($s->notes ? ': ' . $s->notes : ''), 'time' => $s->acted_at?->format('d/m/Y H:i'), 'done' => $s->action !== 'PENDING'])->all()" />
            </div>
        </details>
        @empty
...
<x-ui.card title="Riwayat Pengajuan" class="mt-4">
    <x-ui.table>
        ...
        <tr class="hover:bg-slate-50 dark:hover:bg-white/5">
            ...
            <td class="px-4 py-2.5 text-xs">{{ \App\Support\HumanLabel::label($r->module) }}</td>
