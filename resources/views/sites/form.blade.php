...
@section('title', ' - Site Tambang')

@section('content')
<div class="mb-4">
    <h1 class="text-xl font-bold text-slate-800">{{ $item ? 'Edit' : 'Tambah' }} Site Tambang</h1>
</div>

<form method="POST" action="{{ $item ? route('sites.update', $item) : route('sites.store') }}" class="bg-white rounded-xl border border-slate-200 p-6 max-w-4xl">
    @csrf
    ...
    <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="type" required class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm focus:border-amber-400 outline-none">
                        <option value="">-- Pilih --</option>
                        <option value="MINE" @selected($item->type ?? old('type') == 'MINE')>Mine</option>
                        <option value="PLANT" @selected($item->type ?? old('type') == 'PLANT')>Plant</option>
                        <option value="PORT" @selected($item->type ?? old('type') == 'PORT')>Port</option>
                        <option value="OFFICE" @selected($item->type ?? old('type') == 'OFFICE')>Kantor</option>
                    </select>
                </div>
    ...
    <div class="flex gap-2 mt-6">
        ...
        <a href="{{ route('sites.index') }}" class="px-5 py-2 rounded-lg bg-slate-100 text-sm">Batal</a>
    </div>
</form>
@endsection