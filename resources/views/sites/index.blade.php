...
<div class="flex items-center justify-between mb-4">
    <div>
        <h1 class="text-xl font-bold text-slate-800">Site Tambang</h1>
        <p class="text-sm text-slate-500">Kelola data Site Tambang</p>
    </div>
    @can('sites.create')
    <x-btn-create label="Tambah Site Tambang" :href="route('sites.create')" />
    @endcan
</div>

<x-filter-bar :route="route('sites.index')">
    ...
    <tbody>
        ...
        <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-4 py-2.5 text-right whitespace-nowrap">
                            @can('sites.update')<a href="{{ route('sites.edit', $item) }}" class="text-indigo-600 hover:underline text-xs">Edit</a>@endcan
                            @can('sites.delete')
                            <form action="{{ route('sites.destroy', $item) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data ini?')">
                                @csrf @method('DELETE')
