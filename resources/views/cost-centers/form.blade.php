...
@section('content')
<div class="mb-4">
    <h1 class="text-xl font-bold text-slate-800">{{ $item ? 'Edit' : 'Tambah' }} Pusat Biaya</h1>
</div>

<form method="POST" action="{{ $item ? route('cost-centers.update', $item) : route('cost-centers.store') }}" class="bg-white rounded-xl border border-slate-200 p-6 max-w-4xl">
    @csrf
    ...
    <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="type"  class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm focus:border-amber-400 outline-none">
                        <option value="">-- Pilih --</option>
                        <option value="COST" @selected($item->type ?? old('type') == 'COST')>Pusat Biaya</option>
                        <option value="PROFIT" @selected($item->type ?? old('type') == 'PROFIT')>Pusat Laba</option>
                    </select>
                </div>
    ...
    <div class="flex gap-2 mt-6">
        ...
        <a href="{{ route('cost-centers.index') }}" class="px-5 py-2 rounded-lg bg-slate-100 text-sm">Batal</a>
    </div>
</form>
@endsection