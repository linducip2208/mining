...
@section('title', ' - Rute Hauling')

@section('content')
<div class="mb-4">
    <h1 class="text-xl font-bold text-slate-800">{{ $item ? 'Edit' : 'Tambah' }} Rute Hauling</h1>
</div>

<form method="POST" action="{{ $item ? route('hauling-routes.update', $item) : route('hauling-routes.store') }}" class="bg-white rounded-xl border border-slate-200 p-6 max-w-3xl">
    @csrf
    ...
    <div class="grid md:grid-cols-2 gap-4">
        ...
        <div>
            <label class="text-xs font-semibold text-slate-600 uppercase">Jarak (Km)</label>
            <input type="number" step="0.01" min="0" name="distance_km" value="{{ $item->distance_km ?? old('distance_km') }}" class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-amber-400 focus:ring-2 focus:ring-amber-100 outline-none">
        </div>
        <div>
        ...
        <div>
            ...
            <select name="loading_point_id" class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm focus:border-amber-400 outline-none">
                <option value="">-- Pilih --</option>
                @foreach (\App\Models\LoadingPoint::orderBy('name')->get() as $lp)
                    <option value="{{ $lp->id }}" @selected(($item->loading_point_id ?? old('loading_point_id')) == $lp->id)>{{ $lp->name }}</option>
                @endforeach
            </select>
        </div>
        <div>
            <label class="text-xs font-semibold text-slate-600 uppercase">Titik Bongkar</label>
            <select name="dumping_point_id" class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm focus:border-amber-400 outline-none">
                <option value="">-- Pilih --</option>
                @foreach (\App\Models\DumpingPoint::orderBy('name')->get() as $dp)
                    <option value="{{ $dp->id }}" @selected(($item->dumping_point_id ?? old('dumping_point_id')) == $dp->id)>{{ $dp->name }}</option>
                @endforeach
            </select>
        </div>
        <div>
    ...
    <div class="flex gap-2 mt-6">
        ...
        <a href="{{ route('hauling-routes.index') }}" class="px-5 py-2 rounded-lg bg-slate-100 text-sm">Batal</a>
    </div>
</form>
@endsection
