...
@section('content')
<div class="mb-4">
    <h1 class="text-xl font-bold text-slate-800">{{ $item ? 'Edit' : 'Tambah' }} Supplier</h1>
</div>

<form method="POST" action="{{ $item ? route('suppliers.update', $item) : route('suppliers.store') }}" class="bg-white rounded-xl border border-slate-200 p-6 max-w-4xl">
    @csrf
    ...
    <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="type" required class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm focus:border-amber-400 outline-none">
                        <option value="">-- Pilih --</option>
                        <option value="VENDOR" @selected($item->type ?? old('type') == 'VENDOR')>Vendor</option>
                        <option value="SERVICE" @selected($item->type ?? old('type') == 'SERVICE')>Jasa</option>
                        <option value="TRANSPORT" @selected($item->type ?? old('type') == 'TRANSPORT')>Transportir</option>
                    </select>
                </div>
    ...
    <div class="flex gap-2 mt-6">
        ...
        <a href="{{ route('suppliers.index') }}" class="px-5 py-2 rounded-lg bg-slate-100 text-sm">Batal</a>
    </div>
</form>
@endsection