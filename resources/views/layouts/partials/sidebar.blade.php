@php
    $groups = [
        ...
        ['label' => 'Keuangan', 'icon' => 'bank', 'items' => [
            ['label' => 'Cash & Bank', 'route' => 'cash-accounts.index', 'active' => 'cash-accounts.*', 'perm' => 'finance.view', 'icon' => 'bank'], ['label' => 'Laba Rugi', 'route' => 'finance.pl', 'active' => 'finance.pl', 'perm' => 'ledger.view', 'icon' => 'chart'], ['label' => 'Budget', 'route' => 'budgets.index', 'active' => 'budgets.*', 'perm' => 'budget.view', 'icon' => 'wallet']]],
        ...
        ['label' => 'Administrasi', 'icon' => 'sliders', 'items' => [['label' => 'Pengguna', 'route' => 'users.index', 'active' => 'users.*', 'perm' => 'user.view', 'icon' => 'users'], ['label' => 'Peran & Izin', 'route' => 'role.index', 'active' => 'role.*', 'perm' => 'role.view', 'icon' => 'shield'], ['label' => 'Pengaturan', 'route' => 'setting.index', 'active' => 'setting.*', 'perm' => 'setting.view', 'icon' => 'cog']]],
    ];
...
<aside id="sidebar" aria-label="Navigasi utama" class="fixed inset-y-0 left-0 z-30 flex flex-col bg-[#101923] text-slate-300 transition-[width,transform] duration-200 w-60 -translate-x-full md:translate-x-0">
    ...
    <nav class="flex-1 overflow-y-auto nice-scroll px-2 pb-4 text-[13px]" aria-label="Menu modul">
        ...
        @foreach ($groups as $group)
            @php $visible = collect($group['items'])->filter(fn ($i) => auth()->user()->hasPermission($i['perm'])); $active = $visible->contains(fn ($i) => $isActive($i['active'])); @endphp
