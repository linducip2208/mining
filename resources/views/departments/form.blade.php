...
@section('content')
<div class="mb-4">
    <h1 class="text-xl font-bold text-slate-800">{{ $item ? 'Edit' : 'Tambah' }} Departemen</h1>
</div>

<form method="POST" action="{{ $item ? route('departments.update', $item) : route('departments.store') }}" class="bg-white rounded-xl border border-slate-200 p-6 max-w-4xl">
    @csrf
    ...
    <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="text-xs font-semibold text-slate-600 uppercase">Divisi</label>
                    <select name="division_id"  class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm focus:border-amber-400 outline-none">
                        <option value="">-- Pilih --</option>
                        @foreach ($divisions ?? [] as $id => $n)
                            <option value="{{ $id }}" @selected($item->division_id ?? old('division_id'))>{{ $n }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
    ...
    <div class="flex gap-2 mt-6">
        ...
        <a href="{{ route('departments.index') }}" class="px-5 py-2 rounded-lg bg-slate-100 text-sm">Batal</a>
    </div>
</form>
@endsection