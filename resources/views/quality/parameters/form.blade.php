...
@section('content')
<div class="mb-4">
    <h1 class="text-xl font-bold text-slate-800">{{ $item ? 'Edit' : 'Tambah' }} Parameter Uji</h1>
</div>

<form method="POST" action="{{ $item ? route('quality-parameters.update', $item) : route('quality-parameters.store') }}" class="bg-white rounded-xl border border-slate-200 p-6 max-w-3xl">
    @csrf
    ...
    <div class="grid md:grid-cols-2 gap-4">
        <div>
            ...
            <input type="text" name="code" value="{{ $item->code ?? old('code') }}" required maxlength="30" placeholder="TM / TS / ASH / CV" class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-amber-400 focus:ring-2 focus:ring-amber-100 outline-none">
        </div>
        <div>
        ...
        <div>
            ...
            <input type="text" name="unit" value="{{ $item->unit ?? old('unit') }}" maxlength="20" placeholder="% / kcal/kg / ppm" class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-amber-400 focus:ring-2 focus:ring-amber-100 outline-none">
        </div>
        <div>
    ...
    <div class="flex gap-2 mt-6">
        ...
        <a href="{{ route('quality-parameters.index') }}" class="px-5 py-2 rounded-lg bg-slate-100 text-sm">Batal</a>
    </div>
</form>
@endsection
