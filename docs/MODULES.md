...
## 21. Administrasi Register (Surat, Invoice, Kwitansi)
Register Surat dengan jenis configurable (`letter_types`), penomoran per-jenis via NumberingService (`{SEQ}/{TYPE}-{COMPANY}/{MONTH_ROMAN}/{YEAR}`, reset YEARLY/MONTHLY/NEVER), reservasi nomor (RESERVED/USED/CANCELLED/VOID, tak dipakai ulang), workflow DRAFT→NUMBER_RESERVED→REVIEW→APPROVED→SIGNED→SENT→ARCHIVED (atau DRAFT→PUBLISHED→ARCHIVED), approval via engine, lampiran private, relasi polimorfik ke SO/DO/Invoice. Register Invoice adalah view atas Sales Invoice (status ISSUED/LUNAS/OVERDUE turunan, riwayat cetak dari audit). Kwitansi wajib dari payment POSTED (tanpa uang-masuk ganda), penomoran `KW/...`, workflow DRAFT→ISSUED→CONFIRMED→VOID. Terbilang Indonesia terpusat (`NumberToWordsService`).

## 22. Gudang Sparepart
Master memakai Item `type=SPAREPART` (brand, part number, lokasi rak, min/max/reorder, supplier preferensi) — tanpa tabel stok ganda. Hierarki lokasi Warehouse→Zone→Rack→Bin (`storage_locations`). Barang masuk/keluar adalah facade di atas `StockService::move` (tipe SPAREPART_IN/OUT, OPENING untuk stok lama); keluar untuk WO memakai `MaintenanceService::issuePart` (stok + cost + jurnal Dr Maintenance/Cr Sparepart). Reservasi concurrency-safe (row lock) dengan issued/returned tracking; return wajib referensi. Kartu stok 100% dari ledger. Opname COUNTING→REVIEW→approval→POSTED dengan jurnal selisih (VARIANCE_*). Rekomendasi PR (`max − available`) membuat PR draft — tanpa auto-beli. Kompatibilitas opsional, scan barcode sebagai input keyboard, import legacy idempoten. Koreksi hanya via reversal/adjustment — ledger tak pernah di-UPDATE/DELETE.
