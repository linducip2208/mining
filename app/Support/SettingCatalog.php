...
final class SettingCatalog
{
    public static function all(): array
    {
        ...
        $add('printer.default_a4_printer_id', ['label' => 'Printer A4 Default', 'description' => 'Printer utama untuk laporan, invoice, dan dokumen kantor.', 'group' => 'Printer & Perangkat', 'type' => 'model_select', 'model' => 'printer', 'label_column' => 'name', 'value_column' => 'id', 'default' => null, 'validation' => 'nullable|integer']);
        $add('printer.default_thermal_printer_id', ['label' => 'Printer Thermal Default', 'description' => 'Printer utama untuk struk dan tiket berukuran thermal.', 'group' => 'Printer & Perangkat', 'type' => 'model_select', 'model' => 'printer', 'label_column' => 'name', 'value_column' => 'id', 'default' => null, 'validation' => 'nullable|integer']);
        $add('printer.default_weighbridge_printer_id', ['label' => 'Printer Timbangan Default', 'description' => 'Printer otomatis untuk tiket timbang setelah timbang kedua.', 'group' => 'Printer & Perangkat', 'type' => 'model_select', 'model' => 'printer', 'label_column' => 'name', 'value_column' => 'id', 'default' => null, 'validation' => 'nullable|integer']);
        $select('printer.thermal_width', 'Lebar Thermal', 'Lebar kertas thermal untuk tiket dan struk.', 'Printer & Perangkat', '80mm', ['58mm' => '58 mm', '80mm' => '80 mm']);
        $bool('printer.auto_print_weighbridge', 'Auto Print Tiket Timbangan', 'Masukkan tiket timbang yang selesai ke antrean printer lokal.', 'Printer & Perangkat', false);
        $bool('printer.auto_cut', 'Potong Kertas Otomatis', 'Kirim perintah potong jika printer mendukung ESC/POS.', 'Printer & Perangkat', true);
